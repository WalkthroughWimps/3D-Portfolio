...
const SF2_LIB_CACHE_BUST = '2026-01-05-2';
const SF2_LIB_CANDIDATES = [
  {
    label: 'js-synthesizer-local',
    scripts: [
  ...
  {
    label: 'js-synthesizer-local-min',
    scripts: [
      '/assets/vendor/js-synthesizer/externals/libfluidsynth-2.4.6.js',
      '/assets/vendor/js-synthesizer/dist/js-synthesizer.min.js'
    ]
  },
  {
    label: 'js-synthesizer-libsndfile',
    scripts: [
      '/assets/vendor/js-synthesizer/externals/libfluidsynth-2.4.6-with-libsndfile.js',
      '/assets/vendor/js-synthesizer/dist/js-synthesizer.js'
    ]
  },
  {
    label: 'js-synthesizer-libsndfile-min',
    scripts: [
      '/assets/vendor/js-synthesizer/externals/libfluidsynth-2.4.6-with-libsndfile.js',
      '/assets/vendor/js-synthesizer/dist/js-synthesizer.min.js'
    ]
...
console.log('[SF2] loader revision', SF2_LIB_CACHE_BUST);
...
function getSf2ApiCandidate(){
  const candidates = [
    ...
    ['JSSynth.Synthesizer', window.JSSynth && window.JSSynth.Synthesizer],
...
function appendCacheBuster(url){
  ...
  return `${url}${sep}v=${encodeURIComponent(SF2_LIB_CACHE_BUST)}`;
}

async function ensureSf2SynthLibrary(){
  if(getSf2ApiCandidate()) return true;
  for(const candidate of SF2_LIB_CANDIDATES){
    const scripts = Array.isArray(candidate)
      ? candidate
      : (candidate && candidate.scripts ? candidate.scripts : [candidate]).flat().filter(Boolean);
    if(!scripts.length) continue;
    const label = candidate && candidate.label ? candidate.label : 'sf2';
    console.log('[SF2] loading scripts', { label, scripts });
    let allOk = true;
    for(const url of scripts){
      const scriptUrl = appendCacheBuster(url);
      const ok = await loadLocalScriptOnce(scriptUrl);
      if(!ok){ allOk = false; break; }
    }
    if(allOk && getSf2ApiCandidate()) return true;
  }
  return false;
}

async function ensureSf2SynthReady(apiInfo){
  if(sf2SynthState.ready) return sf2SynthState.ready;
  const api = apiInfo && apiInfo.api ? apiInfo.api : null;
  if(!api || typeof api.waitForReady !== 'function'){
    sf2SynthState.ready = Promise.resolve(true);
    return sf2SynthState.ready;
  }
  sf2SynthState.ready = Promise.resolve()
    .then(() => api.waitForReady())
    .then(() => true)
    .catch((err) => {
      console.warn('[SF2] waitForReady failed', err);
      return false;
    });
  return sf2SynthState.ready;
}
...
async function loadSf2InstrumentForConfig(config){
  ...
  if(!ok){
    console.warn('[SF2] synth library missing. Provide assets/vendor/js-synthesizer/dist/js-synthesizer(.min).js plus externals/libfluidsynth-2.4.6.js');
    return null;
  }
  ...
  const ready = await ensureSf2SynthReady(apiInfo);
  if(!ready){
    console.warn('[SF2] synth not ready');
    return null;
  }
  ...
  const rawUrl = String(config.sf2Url);
